...
PREPARE_LOGGING(messageconsumer_i)

void messageconsumer_i::messageReceived(const std::string &id,
const Component_Members_struct &msg) {
LOG_INFO(messageconsumer_i, id<<" "<<msg.Float<<" "<<msg.String);
}

messageconsumer_i::messageconsumer_i(const char *uuid, const char *label) :
    messageconsumer_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

messageconsumer_i::~messageconsumer_i()
{
}

void messageconsumer_i::constructor()
{
    /***********************************************************************************
     This is the RH constructor. All properties are properly initialized before this function is called 
    ***********************************************************************************/
	message_in->registerMessage("Component_Members", this, &messageconsumer_i::messageReceived);
}
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void messageconsumer_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &messageconsumer_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &messageconsumer_i::<callback method>)
        in the constructor.
        ...
        //Add to messageconsumer.cpp
        messageconsumer_i::messageconsumer_i(const char *uuid, const char *label) :
            messageconsumer_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &messageconsumer_i::scaleChanged);
            addPropertyListener(status, this, &messageconsumer_i::statusChanged);
        }

        void messageconsumer_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void messageconsumer_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            RH_DEBUG(this->_baseLog, "status changed");
        }
            
        //Add to messageconsumer.h
...
************************************************************************************************/
int messageconsumer_i::serviceFunction()
{
